 ...
 * @author htonus
 */
final class controllerRealtyType extends CommonEditor
{
	...
	public function doEdit(HttpRequest $request)
	{
		...
		if ($this->getForm()->getValue('id')) {
			$list = Criteria::create(RealtyType_i18n::dao())->
				add(
		...
		$i18n = RealtyType_i18n::proto()->getMapping();
	...
	private function saveI18n(RealtyType $subject, $ids, $fields)
	{
		$languageList = Language::dao()->getList();
		
		foreach($ids as $code => $id) {
			if (empty($id)) {
				$i18n = RealtyType_i18n::create()->
					setObject($subject)->
					setLanguage($languageList[$code]);
			} else {
				$i18n = RealtyType_i18n::dao()->getById($id);
			}
